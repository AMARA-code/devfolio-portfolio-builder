...
function TiltCard({ children }: { children: React.ReactNode }) {
  ...
  return (
    <motion.div ref={ref} onMouseMove={onMove} onMouseLeave={onLeave}
      style={{ rotateX, rotateY, transformPerspective: 600 }} className="w-full">
      {children}
    </motion.div>
  )
}
...
/* ─── Glow input ──────────────────────────────────────────────────────────── */
function GlowInput({ label, type, placeholder, value, onChange, required }: {
  ...
  return (
    <div className="relative flex flex-col gap-1.5">
      <label className="text-xs font-semibold tracking-widest uppercase text-violet-300/70">{label}</label>
      <div className="relative">
        <motion.div
          className="absolute inset-0 rounded-xl pointer-events-none"
          animate={focused
            ? { boxShadow: '0 0 0 1.5px rgba(139,92,246,0.7), 0 0 22px rgba(139,92,246,0.22)' }
        ...
        <input
          ...
          className="w-full rounded-xl px-4 py-3 text-sm text-white placeholder-white/20
            bg-white/[0.03] outline-none transition-colors duration-200 font-light tracking-wide"
        />
      </div>
    </div>
  )
  ...
  return (
    <div className="relative flex min-h-screen items-center justify-center p-4 overflow-hidden"
      ...
      <style>{`
        ...
        .github-btn { position: relative; overflow: hidden; }
        .github-btn::after {
          content: ''; position: absolute; inset: 0; border-radius: inherit;
          background: linear-gradient(90deg, transparent, rgba(255,255,255,0.06), transparent);
          background-size: 200% 100%; opacity: 0; transition: opacity 0.3s;
        }
        .github-btn:hover::after { opacity: 1; animation: shimmer 1.4s ease infinite; }
        @keyframes pulse-violet {
          0%, 100% { box-shadow: 0 4px 32px rgba(124,58,237,0.4), inset 0 1px 0 rgba(255,255,255,0.15); }
          50% { box-shadow: 0 4px 48px rgba(124,58,237,0.65), 0 0 0 6px rgba(124,58,237,0.08), inset 0 1px 0 rgba(255,255,255,0.15); }
        }
        .submit-btn:not(:disabled):hover { animation: pulse-violet 2s ease infinite; }
      `}</style>

      <GridLines />
      <Sparks />
      <Orb cx="20%" cy="25%" r="480px" color="radial-gradient(circle, rgba(99,60,200,0.32), transparent 70%)" delay={0} />
      <Orb cx="82%" cy="12%" r="380px" color="radial-gradient(circle, rgba(139,92,246,0.18), transparent 70%)" delay={2.5} />
      <Orb cx="50%" cy="92%" r="430px" color="radial-gradient(circle, rgba(67,56,202,0.22), transparent 70%)" delay={4.5} />
      ...
      >
        ...
        <motion.div
          ...
          transition={{ duration: 0.6, delay: 0.1 }} className="text-center mb-10"
        >
          <Link href="/" className="inline-block group">
            ...
            <motion.div className="mx-auto mt-2 h-px rounded-full"
        ...
        <TiltCard>
          ...
          >
            ...
            <div className="absolute top-0 right-0 w-36 h-36 pointer-events-none"
              style={{ background: 'radial-gradient(circle at top right, rgba(139,92,246,0.1), transparent 70%)' }} />
            ...
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.42 }}>
              <button onClick={handleGithubLogin} disabled={githubLoading}
                className="github-btn w-full flex items-center justify-center gap-3 rounded-xl py-3 px-4
                  text-sm font-medium text-white/75 transition-all duration-300
                  hover:text-white hover:scale-[1.02] active:scale-[0.98] disabled:opacity-50"
                style={{
            ...
            <AnimatePresence>
              {error && (
                ...
                >
                  <span className="text-red-400 font-semibold mr-2">✕</span>{error}
                </motion.div>
              )}
            </AnimatePresence>
            ...
            <form onSubmit={handleEmailLogin} className="flex flex-col gap-4">
              {[
                ...
                <motion.div key={field.label}
                  ...
                  <GlowInput label={field.label} type={field.type} placeholder={field.placeholder}
                    value={field.value} onChange={(e) => field.set(e.target.value)} required />
                </motion.div>
              ))}
              ...
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.68 }}
                ...
                <Link href="/forgot-password"
                  className="text-xs text-violet-400/60 hover:text-violet-300 transition-colors duration-200 relative group">
                  Forgot password?
                  <span className="absolute -bottom-0.5 left-0 h-px w-0 group-hover:w-full transition-all duration-300 bg-violet-400/50" />
                </Link>
              </motion.div>

              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
                transition={{ delay: 0.72 }} className="mt-1">
            ...
            <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.88 }}
